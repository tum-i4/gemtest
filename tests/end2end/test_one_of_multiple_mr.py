...
def dummy_transformation_2(mtc: gmt.MetamorphicTestCase):
...
def dummy_relation_2(mtc: gmt.MetamorphicTestCase):
    return mtc.source_output == pytest.approx(mtc.followup_output)


@pytest.mark.xfail()
@gmt.system_under_test(mr_1, mr_3)
def test_dummy_sut_1(input: float) -> float:
